...
function toEventInput(
  ...
  const normalizedFormat = normalizeEventFormat(event.event_format);
...
function sortAttendeePreview(
  ...
  const relationshipDiff =
    relationshipStrength(right) - relationshipStrength(left);
...
export function buildCommunityNetworkingHomeData({
  ...
  for (const attendee of visibleAttendees) {
    const eventAttendees =
      visibleAttendeesByEventId.get(attendee.eventId) ?? [];
  ...
  const priorityEvents = priorityEventInputs
    .map<NetworkingOpportunityEvent | null>((event) => {
      ...
      return {
        ...
        networkAttendingCount: attendees.filter(
          (attendee) => attendee.isInNetwork,
        ).length,
  ...
  const priorityEventsById = new Map(
    priorityEvents.map((event) => [event.id, event]),
  );
  ...
  for (const attendee of visibleAttendees) {
    ...
    existing.isMutualFollow =
      existing.isMutualFollow || attendee.isMutualFollow;
    ...
    if (
      !existing.sharedEvents.some((sharedEvent) => sharedEvent.id === event.id)
    ) {
  ...
  const meetPeople = Array.from(meetPeopleMap.values())
    .map<NetworkingPersonCard>((person) => {
      const sortedSharedEvents = [...person.sharedEvents].sort(
        (left, right) =>
          getTimeValue(left.startTime) - getTimeValue(right.startTime),
      );
  ...
  const recentEventInputsById = new Map(
    recentEventInputs.map((event) => [event.id, event]),
  );
  ...
  const followUps = Array.from(followUpsMap.values())
    .map<NetworkingFollowUpCard>((person) => {
      const sortedSharedEvents = [...person.sharedEvents].sort(
        (left, right) =>
          getTimeValue(right.startTime) - getTimeValue(left.startTime),
      );
      ...
      return {
        ...
        mostRecentSharedEventStartTime:
          sortedSharedEvents[0]?.startTime ?? null,
        sharedEvents: sortedSharedEvents.slice(0, SHARED_EVENT_PREVIEW_LIMIT),
      };
    })
    .sort(rankFollowUps)
...
export class CommunityNetworkingHomeService {
  ...
  private static scorePastSpeakerMatch({
    ...
    if (currentRole) {
      ...
      const roleOverlap = roleTokens.filter((token) =>
    ...
    for (const interest of interests) {
      ...
      if (interestOverlap.length > 0) {
        ...
        reasons.push({
          ...
          text: `Aligned with your interest in ${formatKeywordLabel(
            matchedToken,
          )}.`,
        });
      }
    ...
    const strongestReason = [...reasons].sort(
  ...
  private static async getPastSpeakerMatches({
    ...
    const pastEvents = ((pastEventsResult.data || []) as EventRow[]).filter(
      (event) =>
        Boolean(event.slug && event.title) && !isShowcaseEventSource(event),
    );
    ...
    const eventsById = new Map<string, EventRow>(
      pastEvents.map((event) => [event.id, event]),
    );
    const seenSpeakerKeysByEventId = new Map<string, Set<string>>();
    const bestMatchesBySpeakerId = new Map<
      string,
      { speaker: NetworkingSpeakerPreview; event: EventRow; score: number; matchReason: string }
    >();
    ...
    for (const row of speakerResult.data as Array<{
      ...
      const speakerKey = row.speaker_id;
      const seenSpeakerKeys =
        seenSpeakerKeysByEventId.get(row.event_id) ?? new Set<string>();
      if (seenSpeakerKeys.has(speakerKey)) {
        continue;
      }
      seenSpeakerKeys.add(speakerKey);
  ...
  private static async getVisibleAttendeeContext({
    ...
    if (followingIds.size > 0) {
      ...
      for (const row of (candidateFollowingResult.data ||
        []) as CandidateFollowingRow[]) {
      ...
      if (mutualProfileIds.size > 0) {
        ...
        for (const [
          userId,
          mutualIds,
        ] of candidateMutualIdsByUserId.entries()) {
          let count = 0;
    ...
    for (const row of attendanceRows) {
      ...
      attendeeInputs.push({
        ...
        mutualConnectionsCount:
          mutualConnectionsCountByUserId.get(row.user_id) ?? 0,
  ...
  private static async getStarterEventData({
    ...
    Pick<CommunityNetworkingHomeData, "priorityEvents" | "meetPeople">
  > {
    ...
    const starterHomeData = buildCommunityNetworkingHomeData({
      ...
      priorityEventInputs: visibleStarterEventRows.map((event) =>
        toEventInput(event),
      ),
      recentEventInputs: [],
      attendeeInputs,
  ...
  private static async getAmbientActivityCounts({
    ...
    Omit<CommunityNetworkingAmbientActivity, "roomsWithFreshTrackingCount">
  > {
  ...
  private static async attachRecentTrackerCounts({
    ...
    const recentTrackingRows = (recentTrackingResult.data ||
      []) as RecentTrackingRow[];
    ...
    if (publicProfilesResult.error) {
      throw new Error(
        "Failed to validate recent community tracking visibility.",
      );
    }
  ...
  private static async attachSpeakerPreviews({
    ...
    let speakerDetailsById = new Map<string, NetworkingSpeakerPreview>();
    if (speakerIds.length > 0) {
      const detailResult = await readClient
        .from("speakers")
        .select(
    ...
    for (const row of speakerResult.data as Array<{
      ...
      const seenSpeakerIds =
        seenSpeakerIdsByEventId.get(row.event_id) ?? new Set<string>();
  ...
  private static async attachAmbientCommunity({
    ...
    const meetPeople =
      homeData.meetPeople.length > 0
        ? homeData.meetPeople
        : starterEventData.meetPeople;
  ...
  static async getHomeData({
    ...
    const attendanceVisibilityEnabled =
      ((viewerProfileResult.data || null) as ViewerProfileRow | null)
        ?.show_attendance === true;

    const viewerEvents = (
      (viewerEventsResult.data || []) as ViewerEventRow[]
    ).filter(
      (row) =>
        row.is_bookmarked ||
    ...
    const eventIds = Array.from(
      new Set(viewerEvents.map((row) => row.event_id)),
    );
    if (eventIds.length === 0) {
    ...
    const nowTime = now.getTime();
    const followUpCutoff =
      nowTime - FOLLOW_UP_WINDOW_DAYS * 24 * 60 * 60 * 1000;
    const eventsById = new Map<string, EventRow>(
    ...
    for (const row of viewerEvents) {
      ...
      if (
        startTime >= nowTime &&
        (row.status === "attending" || row.is_bookmarked)
      ) {
