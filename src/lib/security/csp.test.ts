...
describe('buildCsp', () => {
    ...
    it('defaults production to a nonce-based strict policy when CSP_STAGE is unset', () => {
        vi.stubEnv('NODE_ENV', 'production');
        ...
        expect(scriptSrc).toContain("'nonce-default-nonce'");
        expect(scriptSrc).toContain("'strict-dynamic'");
        expect(scriptSrc).not.toContain("'unsafe-inline'");
        expect(scriptSrc).not.toContain("'unsafe-eval'");
    });

    it('emits a nonce-based strict policy when CSP_STAGE=strict', () => {
        vi.stubEnv('NODE_ENV', 'production');
