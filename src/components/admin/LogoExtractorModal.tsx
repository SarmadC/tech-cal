...
interface LogoExtractorModalProps {
    isOpen: boolean;
    onClose: () => void;
...
export default function LogoExtractorModal({
    isOpen,
    onClose,
    onSelect,
    initialUrl = '',
}: LogoExtractorModalProps) {
    ...
    return (
        <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="logo-extractor-title"
            ...
            <div className="flex max-h-[85vh] w-[min(90vw,800px)] flex-col rounded-xl border border-slate-800 bg-slate-950 shadow-2xl">
                ...
                <div className="flex items-center justify-between border-b border-slate-800 px-5 py-4">
                    <div className="flex items-center gap-3">
                        <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-slate-800">
                            <MaterialIcon name="star" size={18} className="text-slate-300" />
                        </div>
                        <div>
                            <h2 id="logo-extractor-title" className="text-base font-semibold text-slate-100">
                                Extract Logo from URL
                            </h2>
                            <p className="text-xs text-slate-400">
                                Enter a website URL to extract logo candidates
                            </p>
                        </div>
                    </div>
                    <Button
                ...
                <div className="flex-1 overflow-y-auto px-5 py-4">
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-12 text-slate-400">
                            <MaterialIcon name="star" size={48} className="mb-3 opacity-50" />
                            <p className="text-sm">
                                {images.length > 0
                                    ? 'All extracted images failed to load'
                                    : 'Enter a URL and click Extract to find logos'}
                            </p>
                        </div>
                    ) : (
                        <>
                            <p className="mb-3 text-xs text-slate-400">
                                Found {visibleImages.length} images. Click to select a logo.
                            </p>
                    ...
                    <div className="border-t border-slate-800 px-5 py-4">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex-1">
                                <p className="text-sm font-medium text-slate-200">Selected Logo</p>
