...
export interface FirecrawlExtractResponse {
    ...
    error?: string;
    creditsUsed?: number;
    status?: string; // For async job status: 'processing' | 'completed' | 'failed' | 'cancelled'
    id?: string; // Job ID for async operations
    expiresAt?: string; // ISO timestamp for result expiration
}

/**
 * Runtime validation for ExtractedEventData
 */
export function validateExtractedEventData(data: unknown): data is ExtractedEventData {
    if (!data || typeof data !== 'object') {
        return false;
    }

    const extracted = data as Partial<ExtractedEventData>;

    // Validate optional string fields
    if (extracted.description !== undefined && typeof extracted.description !== 'string') {
        return false;
    }
    if (extracted.startTime !== undefined && typeof extracted.startTime !== 'string') {
        return false;
    }
    if (extracted.endTime !== undefined && typeof extracted.endTime !== 'string') {
        return false;
    }
    if (extracted.imageUrl !== undefined && typeof extracted.imageUrl !== 'string') {
        return false;
    }

    // Validate optional array fields
    if (extracted.agenda !== undefined && !Array.isArray(extracted.agenda)) {
        return false;
    }
    if (extracted.speakers !== undefined && !Array.isArray(extracted.speakers)) {
        return false;
    }
    if (extracted.dailySchedule !== undefined && !Array.isArray(extracted.dailySchedule)) {
        return false;
    }

    // Validate optional object fields
    if (extracted.pricing !== undefined && (typeof extracted.pricing !== 'object' || extracted.pricing === null)) {
        return false;
    }
    if (extracted.venue !== undefined && (typeof extracted.venue !== 'object' || extracted.venue === null)) {
        return false;
    }
...
/**
 * Type guard for FirecrawlScrapeResponse
 */
export function isFirecrawlScrapeResponse(data: unknown): data is FirecrawlScrapeResponse {
    if (!data || typeof data !== 'object') {
        return false;
    }

    const response = data as Partial<FirecrawlScrapeResponse>;
    return typeof response.success === 'boolean';
}

/**
 * Type guard for FirecrawlExtractResponse
 */
export function isFirecrawlExtractResponse(data: unknown): data is FirecrawlExtractResponse {
    if (!data || typeof data !== 'object') {
        return false;
    }

    const response = data as Partial<FirecrawlExtractResponse>;
    return typeof response.success === 'boolean';
}

/**
 * Safely extract ExtractedEventData from unknown response
 */
export function extractEventDataFromResponse(data: unknown): ExtractedEventData | null {
    if (!validateExtractedEventData(data)) {
        return null;
    }
    return data;
}
