...
export function useSearchSuggestions({
    ...
    useEffect(() => {
        const hasSearchTerm = Boolean(debouncedSearchTerm && debouncedSearchTerm.length >= 2);

        if (!hasSearchTerm) {
            // Avoid triggering rerenders when nothing to clear
        ...
        const timer = setTimeout(() => {
            ...
            const allTags = stableTags.length > 0 ? stableTags : events.flatMap(e => e.tags || []);
    ...
    }, [debouncedSearchTerm, events, categories, stableTags, maxSuggestions]);
