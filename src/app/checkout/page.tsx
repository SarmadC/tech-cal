...
function CheckoutContent() {
  ...
  const openCheckout = useCallback(async () => {
    ...
    try {
      // Pass the frame target ID for inline checkout
      const frameTarget = 'paddle-checkout-frame';
      if (selectedPlan === 'annual') {
        await openProAnnualCheckout(user.id, user.email || undefined, frameTarget);
      } else {
        await openProMonthlyCheckout(user.id, user.email || undefined, frameTarget);
      }
      // Checkout is now embedded
      setStatus('ready');
    } catch (err) {
        ...
        <div className="bg-background-secondary rounded-2xl border border-border p-8 mb-8">
          ...
          <div className="flex gap-2 mb-6">
            <Link
              ...
              className={`flex-1 text-center py-2 px-4 rounded-lg border transition-colors ${selectedPlan === 'monthly'
            ...
            <Link
              ...
              className={`flex-1 text-center py-2 px-4 rounded-lg border transition-colors ${selectedPlan === 'annual'
                  ? 'bg-accent-primary text-accent-primary-foreground border-accent-primary'
                  : 'border-border text-foreground-secondary hover:border-accent-primary/50'
                }`}
            >
          ...
          <div className="w-full min-h-[450px]">
            {!user ? (
              ...
              <div className="flex flex-col items-center justify-center py-12 text-foreground-secondary">
                <Spinner className="h-8 w-8 animate-spin mb-4 text-accent-primary" />
                <p>Loading secure payment...</p>
              </div>
            ) : (
              <div id="paddle-checkout-frame" className="w-full min-h-[450px] bg-white dark:bg-transparent rounded-lg" />
            )}
          </div>
        </div>
