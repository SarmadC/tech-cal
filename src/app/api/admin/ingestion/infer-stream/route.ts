...
/**
 * POST /api/admin/ingestion/infer-stream
 *
 * SSE streaming endpoint for bulk inference with real-time progress updates.
...
export async function POST(request: NextRequest) {
    try {
        ...
        const stream = new ReadableStream({
            async start(controller) {
                ...
                try {
                    ...
                    Sentry.captureException(error, { extra: { route: 'admin/ingestion/infer-stream' } });
        ...
        const message = error instanceof Error ? error.message : 'Unknown error';
        console.error('Admin inference stream failed:', message);
        Sentry.captureException(error, { extra: { route: 'admin/ingestion/infer-stream' } });
