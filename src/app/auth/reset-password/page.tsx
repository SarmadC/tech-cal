...
const InvalidTokenState = ({ error }: { error: string }) => (
    <div className="min-h-screen bg-background-main flex items-center justify-center py-12 px-4">
        <div className="max-w-md w-full text-center">
            <div className="bg-background-secondary rounded-2xl p-8 border border-border-color">
                ...
                <h1 className="text-2xl font-bold text-foreground-primary mb-4">Invalid Reset Link</h1>
                <p className="text-foreground-secondary mb-6">{error}</p>
                <div className="space-y-3">
                    <Link href="/forgot-password" className="w-full bg-accent-primary hover:bg-accent-primary-hover text-white font-semibold py-3 px-4 rounded-lg transition-all inline-block">
                        Request New Reset Link
                    </Link>
                    <Link href="/login" className="w-full bg-background-main hover:bg-background-tertiary text-foreground-primary font-medium py-3 px-4 rounded-lg transition-all border border-border-color inline-block">
                        Back to Sign In
                    </Link>
                </div>
            </div>
...
export default function ResetPasswordPage() {
    ...
    // QUERY: Verify the reset tokens from the URL.
    const { isLoading: isVerifyingToken, isError: isTokenInvalid, error: tokenError } = useQuery({
        queryKey: ['verifyResetToken', searchParams.toString()],
        queryFn: async () => {
    ...
    const { mutate: updatePassword, isPending, isSuccess, error: mutationError } = useMutation({
        mutationFn: (newPassword: string) => AuthService.updateUserPassword(newPassword),
        onSuccess: () => {
            setTimeout(() => {
    ...
    // Render logic based on query and mutation states
    if (isVerifyingToken) {
        return <LoadingState />;
    }

    if (isTokenInvalid) {
        return <InvalidTokenState error={tokenError.message} />;
    }
