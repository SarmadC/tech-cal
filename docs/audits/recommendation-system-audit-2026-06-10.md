...
> **Remediation status (2026-06-10, same branch):** All findings addressed across two passes.
>
> *Pass 1:* C1, H1, H3, M1, M2 (component breakdown), M3, M4 (expansion + category double-count), M5, M6, M7 (location threading), M8 (hydration + decoration reuse), L2, L5, L6; H2 for the score-breakdown endpoint. Golden masters deliberately re-recorded for the intentional scoring changes.
>
> *Pass 2:* `timingBonus` implemented in the alignment core (gated on existing alignment, scaled by career timeframe; algorithm version bumped to `alignment-core-v2.1` to roll the KV cache). Lookalike fallback cohorts now rank by seniority proximity + skill/interest overlap instead of exact-seniority filtering. UI bucket thresholds consolidated onto `RECOMMENDATION_THRESHOLDS` (baseScorer helpers + manager-justification constants); `SCORE_THRESHOLDS` documented as AdvancedScorer-internal. Recommendations route rate limiter is lazily constructed and fails open on KV outage. The major-org cold-start list moved to `COLD_START_MAJOR_ORGANIZERS` in scoringConfig, documented as a stopgap pending an engagement-based reputation signal (the one item that still needs a data source to fix fully). An env-gated integration test (`tagRetrieval.integration.test.ts`) pins real PostgREST embed-filter semantics — **run live against the production Supabase project: the `!inner` query returns only tag-matching events, and the old non-inner shape demonstrably does not filter (both verified 2026-06-10).**
