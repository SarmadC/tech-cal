...
try {
  ...
  const exportResult = spawnSync(
    npxCommand,
    ['expo', 'export', '--platform', 'ios', '--output-dir', outputDir, '--no-bytecode', '--no-minify'],
    {
      cwd: projectRoot,
      env: {
        ...process.env,
  ...
  for (const value of [
    ...
    PLACEHOLDER_ENV.EXPO_PUBLIC_AUTH_REDIRECT_URI,
  ]) {
  ...
  for (const pattern of [/\benv\.EXPO_PUBLIC_/u, /\bprocess\.env\.EXPO_PUBLIC_/u]) {
    if (pattern.test(bundleContents)) {
      fail(`found non-inlined Expo public env access matching ${pattern}`);
    }
  }
