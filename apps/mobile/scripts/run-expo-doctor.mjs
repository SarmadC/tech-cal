...
const KNOWN_NETWORK_ONLY_CHECKS = new Set([
...
const result = spawnSync(doctorCommand, [], {
  ...
  env: {
    ...process.env,
    EXPO_DOCTOR_WARN_ON_NETWORK_ERRORS:
      process.env.EXPO_DOCTOR_WARN_ON_NETWORK_ERRORS ?? '1',
  },
  encoding: 'utf8',
});
...
const hasKnownNetworkOnlyFailure =
  unexpectedCheckNames.length > 0 &&
  unexpectedCheckNames.every((checkName) => KNOWN_NETWORK_ONLY_CHECKS.has(checkName)) &&
  (combinedOutput.includes('Networking has been disabled') ||
    combinedOutput.includes('getaddrinfo ENOTFOUND exp.host') ||
    combinedOutput.includes('This check requires a connection to the Expo API.'));
...
if (hasKnownNetworkOnlyFailure && !hasRealDoctorFindings) {
  process.stderr.write(
    'expo-doctor reported only offline Expo API validation failures; continuing.\n'
  );
  process.exit(0);
}
