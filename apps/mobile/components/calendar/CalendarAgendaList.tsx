...
export function buildAgendaSections(events: MobileCalendarEvent[]): AgendaSection[] {
  const grouped = new Map<string, MobileCalendarEvent[]>();

  events
    .slice()
    .sort((left, right) => new Date(left.startTime).getTime() - new Date(right.startTime).getTime())
    .forEach((event) => {
      const dateKey = formatEventDateKey(event.startTime, event.endTime, event.timezone);
...
export function resolveInitialAgendaSectionIndex(
  ...
  const todaySectionIndex = sections.findIndex((section) => section.dateKey === todayDateKey);

  if (todaySectionIndex >= 0 && hasUpcomingEventInSection(sections[todaySectionIndex]!, now)) {
    return todaySectionIndex;
  }

  const nextUpcomingSectionIndex = sections.findIndex((section) => hasUpcomingEventInSection(section, now));
...
export function CalendarAgendaList({
  ...
  const listRef = useRef<SectionList<MobileCalendarEvent, AgendaSection> | null>(null);
  const initialMonthStartRef = useRef(resolveMonthStartKey(new Date()));
  const hasAutoScrolledRef = useRef(false);

  const sections = useMemo<AgendaSection[]>(() => buildAgendaSections(events), [events]);
              ...
              <View style={styles.eventRowWrap}>
                ...
                >
                  <View style={styles.timeColumn}>
                    ...
                    >
                      {formatEventStartTimeLabel(item.startTime, item.endTime, item.timezone)}
                    </Text>
                  </View>
