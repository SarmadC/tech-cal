...
export function CalendarMonthGrid({
  ...
  const monthDate = useMemo(() => parseLocalDateKey(monthStart) ?? new Date(), [monthStart]);
  ...
  const dayDots = useMemo(() => {
    ...
    events.forEach((event) => {
      const dayKey = formatEventDateKey(event.startTime, event.endTime, event.timezone);
      const color = eventTypeColors[event.eventTypeId] ?? tokens.colors.accent;
  ...
  return (
    ...
    >
      <View style={styles.monthRow}>
        ...
        >
          <FontAwesome name="angle-left" size={14} color={tokens.colors.discoverTextSoft} />
        </Pressable>
        ...
        >
          <FontAwesome name="angle-right" size={14} color={tokens.colors.discoverTextSoft} />
        </Pressable>
      </View>
                    ...
                    {dots.map((color) => (
                      <View
                        ...
                        style={[
                          styles.dot,
                          {
                            backgroundColor: isSelected ? tokens.colors.textInverse : color,
