...
function buildDateKeyFromParts(
  ...
  if (options?.useUtc) {
    return `${date.getUTCFullYear()}-${padDatePart(date.getUTCMonth() + 1)}-${padDatePart(date.getUTCDate())}` as LocalCalendarDateKey;
  }
  ...
  if (options?.timeZone) {
    try {
      ...
      if (year && month && day) {
        return `${year}-${month}-${day}` as LocalCalendarDateKey;
      }
    } catch {
...
export function resolveMonthEndKey(
...
export function resolveDateInMonth(
  selectedDateKey: LocalCalendarDateKey | null,
  monthStartKey: LocalCalendarDateKey
): LocalCalendarDateKey {
  const monthStart = parseLocalDateKey(monthStartKey) ?? new Date();
...
export function formatMonthLabel(value: LocalCalendarDateKey | Date): string {
  const date = typeof value === 'string' ? parseLocalDateKey(value) : value;
  const resolved = date ?? new Date();
...
export function applyRangeSelection(
  range: MobileDiscoverDateRange,
  nextKey: LocalCalendarDateKey
): MobileDiscoverDateRange {
...
export function applySingleSelection(
  current: LocalCalendarDateKey | null,
  nextKey: LocalCalendarDateKey
) {
  if (current === nextKey) {
    return null;
